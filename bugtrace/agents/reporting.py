...
class ReportingAgent(BaseAgent):
    ...
    async def generate_all_deliverables(self) -> Dict[str, Path]:
        ...
        # ================================================================
        # Generate report artifacts BEFORE LLM enrichment.
        #
        # This is intentionally placed before _enrich_findings_batch().
        # If OpenRouter/LLM enrichment is slow, times out, or fails,
        # the user will still have usable JSON/Markdown/HTML reports.
        # ================================================================

        try:
            logger.info(
                f"[{self.name}] Generating baseline report before LLM enrichment"
            )

            baseline_stats = self._calculate_scan_stats(all_findings)

            baseline_paths = self._generate_json_reports(
                all_findings,
                categorized
            )

            baseline_paths.update(
                self._generate_markdown_reports(categorized)
            )
            ...
            baseline_paths.update(
                self._generate_html_report(baseline_paths)
            )

            logger.info(
                f"[{self.name}] Baseline report checkpoint created: "
                f"{len(baseline_paths)} deliverables"
            )

        except Exception as e:
            logger.error(
                f"[{self.name}] Baseline report generation failed: {e}",
                exc_info=True
            )
    ...
    def _meets_report_quality(self, finding: Dict) -> bool:
        ...
        if vuln_type == "SQLI":
            ...
            # UNION-based SQLi confirmation.
            #
            # SQLiAgent can independently confirm UNION-based injection
            # during Phase B/L2 exploitation. A confirmed UNION finding
            # should be treated as solid exploitation evidence when the
            # finding is explicitly VALIDATED_CONFIRMED and contains a
            # working payload.
            sqli_metadata = finding.get("sqli_metadata") or {}

            injection_type = str(
                finding.get("injection_type")
                or sqli_metadata.get("injection_type")
                or evidence.get("injection_type")
                or ""
            ).lower()

            working_payload = (
                finding.get("working_payload")
                or finding.get("payload")
                or evidence.get("working_payload")
                or evidence.get("payload")
            )

            has_union_confirmed = (
                finding.get("type") == "SQLI"
                and finding.get("status") == ValidationStatus.VALIDATED_CONFIRMED.value
                and "union" in injection_type
                and bool(working_payload)
            )
            ...
            if not has_solid_evidence:
                logger.info(
                    ...
                    f"lacks solid exploitation evidence "
                    f"(SQLMap/data/OOB/error/time/boolean/UNION), "
